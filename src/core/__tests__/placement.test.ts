...
const fullTray: Record<PieceTypeId, number> = { "2x2": 5, "2x3": 5, "2x4": 5 };

describe("canPlace", () => {
  it("rejects a brick straddling the board edge", () => {
    const result = canPlace([], board, fullTray, "2x3", 0, { x: 0, y: 0, z: 5 });
  ...
  it("rejects overlap with an existing brick", () => {
    ...
    const result = canPlace(placed, board, fullTray, "2x2", 0, { x: 1, y: 0, z: 1 });
  ...
  it("rejects a brick with one cell over a hole", () => {
    ...
    const result = canPlace(placed, board, fullTray, "2x3", 0, { x: 0, y: 1, z: 0 });
    expect(result).toEqual({ ok: false, reason: "unsupported" });
  });

  it("rejects placing with tray count 0 even when the space is legal", () => {
    const emptyTray: Record<PieceTypeId, number> = { "2x2": 0, "2x3": 5, "2x4": 5 };
    const result = canPlace([], board, emptyTray, "2x2", 0, { x: 0, y: 0, z: 0 });
    expect(result).toEqual({ ok: false, reason: "none-left" });
  });

  it("accepts a legal placement resting on the plate", () => {
    const result = canPlace([], board, fullTray, "2x4", 0, { x: 0, y: 0, z: 0 });
    expect(result).toEqual({ ok: true });
  });

  it("accepts a legal placement fully supported by another brick", () => {
    const placed: Placement[] = [
      { instanceId: "a", typeId: "2x4", rotation: 0, origin: { x: 0, y: 0, z: 0 } },
    ];
    const result = canPlace(placed, board, fullTray, "2x2", 0, { x: 0, y: 1, z: 0 });
    expect(result).toEqual({ ok: true });
  });

  it("checks failures in order: none-left before out-of-bounds", () => {
    const emptyTray: Record<PieceTypeId, number> = { "2x2": 0, "2x3": 5, "2x4": 5 };
    const result = canPlace([], board, emptyTray, "2x2", 0, { x: 100, y: 0, z: 100 });
    expect(result).toEqual({ ok: false, reason: "none-left" });
  });
});
...
describe("canRemove", () => {
  it("rejects removing a brick on the plate that another brick rests on", () => {
    const placed: Placement[] = [
      { instanceId: "a", typeId: "2x4", rotation: 0, origin: { x: 0, y: 0, z: 0 } },
      { instanceId: "b", typeId: "2x2", rotation: 0, origin: { x: 0, y: 1, z: 0 } },
    ];
